...
| Repo-map binary classifier (NUL anywhere; first 8,192 units with >30% outside printable ASCII plus TAB/LF/CR) | conservative survey heuristic | Preserves raw-byte evidence, applies the same rule to string doubles, and only withholds meaningless line/symbol metadata; the map discloses the exact rule and keeps every file directly fetchable |
